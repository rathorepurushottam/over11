...
const ListingItem = ({title, info, button, border}) => {
  ...
  (kycDetails?.pan_verified == 0 && kycDetails?.adhar_verified == 0 && kycDetails?.upi_verified == 0 &&  kycDetails?.bank_verified == 0) ? (
    <Button
  ...
  (kycDetails?.adhar_verified == 1 && kycDetails?.pan_verified == 1 &&  kycDetails?.bank_verified != 1 ||  kycDetails?.bank_verified != 1 ) ? (
    <Button
      onPress={() => NavigationService.navigate(KYC_SCREEN)}
      style={{
        width: 145,
